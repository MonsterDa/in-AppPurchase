...
+ (void)showWithText:(NSString*)text WithDurations:(CGFloat)duration{
    ...
    CGSize size =  [ZJCustomHud makeSize:text font:14 width:[UIScreen mainScreen].bounds.size.width*0.42 hight:0];
    CGFloat h = 40;
...
//    }
    //添加提示框
    UILabel * label = [[UILabel alloc]initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width/2-[UIScreen mainScreen].bounds.size.width*0.42/2, [UIScreen mainScreen].bounds.size.height/2-20, [UIScreen mainScreen].bounds.size.width*0.42, h)];
...
//    [heartImageView startAnimating];
    ...
    UILabel * label = [[UILabel alloc]initWithFrame:CGRectMake(customView.frame.size.width/2-50, 40, 100, 40)];
    label.text = text;
    label.textAlignment = NSTextAlignmentCenter;
